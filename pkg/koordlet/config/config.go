...
func (c *Configuration) InitFlags(fs *flag.FlagSet) {
	...
	fs.StringVar(&c.CPUBurstAllowlistPath, "cpu-burst-allowlist-path", DefaultCPUBurstAllowlistPath, "The path to the CPU burst allowlist config file (mounted from a ConfigMap). If empty or the file does not exist, no pods will receive CPU burst.")
